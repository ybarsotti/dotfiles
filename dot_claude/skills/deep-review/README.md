...
/deep-review security-focused             # all 10 personas wear security hats
/deep-review adversarial-debate           # 5 dimensions × approver-vs-rejecter pairs

# 4. Review a specific PR or file
/deep-review --scope PR-1234              # GitHub PR (uses `gh pr diff`)
/deep-review --scope file:src/auth.ts     # single file vs main
/deep-review --scope feature-x..feature-y # arbitrary git range
```

Reports are saved to `~/.claude/deep-review-runs/<RUN_ID>/report.md`.

## Architecture

```
slash command (~/.claude/commands/deep-review.md)
   │
   ▼
...
All reviewers run **headless in background** (no cmux panes, no UI clutter). Claude-side reviewers and the aggregator both run on **Sonnet**; the Codex side runs on Codex. Each reviewer's prompt is a **fixed file** at `personas/<id>.md` — the orchestrator never writes reviewer prompts; the dispatcher loads the file and appends the variant's role/focus + output schema. After aggregation, the skill runs a `/simplify` cleanup pass on the reviewed scope.

## Variants

| Variant | When to use |
|---------|-------------|
| `default` | General PR review with 13 fixed lenses — `project-patterns` (CLAUDE.md + rules), `docs-consistency` (docs/ stale-check), `db-performance` (EXPLAIN-driven), `senior-frontend`, `senior-backend`, `security`, `edge-cases`, `test-coverage`, `architecture`, `concurrency-races`, `simplicity`, `scope-completeness`, `error-handling-observability` |
...
Variant files are plain YAML at `~/.claude/skills/deep-review/variants/*.yml`. Add your own — the dispatcher picks them up automatically by filename.
...
├── README.md                         # this file
├── SKILL.md                          # protocol Claude reads when /deep-review runs
├── scripts/
...
Reports persist at `~/.claude/deep-review-runs/<RUN_ID>/report.md` (the run dir at `/tmp/deep-review/...` is cleaned up unless `--keep-artifacts`).
