...
The Claude side runs on Sonnet; the Codex side runs `gpt-6-sol` with `model_reasoning_effort=medium` (ignores the `~/.codex/config.toml` default, so review depth doesn't drift with your interactive config). Override per run:
