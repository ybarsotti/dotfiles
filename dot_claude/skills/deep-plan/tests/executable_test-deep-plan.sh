...
PARSE_SAMPLE=$("$PARSE_ARGS" "--ticket PROJ-1 --no-codex build X")
assert_eq "$(jq -r '.ticket' <<<"$PARSE_SAMPLE")" "PROJ-1" "parse-args: --ticket value"
...
PARSE_REPEATED=$("$PARSE_ARGS" "--ticket PROJ-1 --ticket PROJ-2 build X")
assert_eq "$(jq -r '.ticket' <<<"$PARSE_REPEATED")" "PROJ-2" \
