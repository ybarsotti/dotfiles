...
# Extract sections. The PR body is objective — it describes WHAT we solve and WHY,
# never a list of changed files or counts.
CONTEXT=$(awk '/^## Context/{flag=1; next} /^## /{flag=0} flag' "$PLAN")
MERMAID=$(awk '/^## Flow diagram/{flag=1; next} /^## /{flag=0} flag' "$PLAN")
RATIONALE=$(awk '/^## Rationale & key decisions/{flag=1; next} /^## /{flag=0} flag' "$PLAN")
# Fall back to the abstractions decision log when the rationale section is thin.
[ -z "$(echo "$RATIONALE" | tr -d '[:space:]')" ] && \
  RATIONALE=$(awk '/^## Abstractions decision log/{flag=1; next} /^## /{flag=0} flag' "$PLAN")
VERIFICATION=$(awk '/^## Verification/{flag=1; next} /^## /{flag=0} flag' "$PLAN")

# Populate template. {{TITLE}} is not in the template (title is a gh flag).
sed \
  -e "s|{{TICKET}}|${TICKET:-(no ticket)}|g" \
  "$TEMPLATE" > "$BODY.tmp"

# Inject multiline sections (sed multiline replacement is fragile; use awk).
awk -v ctx="$CONTEXT" -v mer="$MERMAID" -v rat="$RATIONALE" -v ver="$VERIFICATION" '
{
  gsub(/\{\{CONTEXT\}\}/, ctx)
  gsub(/\{\{MERMAID\}\}/, mer)
  gsub(/\{\{RATIONALE\}\}/, rat)
  gsub(/\{\{VERIFICATION\}\}/, ver)
  print
}' "$BODY.tmp" > "$BODY"
rm -f "$BODY.tmp"
...
# Assign the PR to the current user (@me).
LABEL_FLAGS=("--label" "$PR_TYPE")
URL=$(gh pr create $DRAFT_FLAG --title "$TITLE" --body-file "$BODY" --assignee @me \
  ...
  URL=$(gh pr create $DRAFT_FLAG --title "$TITLE" --body-file "$BODY" --assignee @me 2>&1) || {
