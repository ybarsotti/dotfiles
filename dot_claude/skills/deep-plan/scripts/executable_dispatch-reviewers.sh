#!/usr/bin/env bash
# dispatch-reviewers.sh — fan out 5 persona reviewers in parallel against the current plan.
#
...
#   <run-dir>/verdict-ticket-matcher-iter<N>.json
...
# Reviewers always run on Claude/Sonnet or Codex (never Opus — Opus is for planners).
# Defaults: architect + project-developer + ticket-matcher on Claude/Sonnet; flow-mapper + qa on Codex.
# With --no-codex, all run on Claude/Sonnet.
declare -a PERSONAS=("architect" "project-developer" "ticket-matcher" "flow-mapper" "qa")
declare -A RUNNER_FOR=(
  ["architect"]="claude:sonnet"
  ["project-developer"]="claude:sonnet"
  ["ticket-matcher"]="claude:sonnet"
  ["flow-mapper"]="codex:"
  ["qa"]="codex:"
)

if [ "$NO_CODEX" = "--no-codex" ]; then
  RUNNER_FOR["flow-mapper"]="claude:sonnet"
  RUNNER_FOR["qa"]="claude:sonnet"
fi
